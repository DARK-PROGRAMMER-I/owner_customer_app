...
class _CreditBookState extends State<CreditBook> {
  ...
  Widget processListItem(
      BuildContext context, int index, List<Customer> customerList) {
        ...
        children: <Widget>[
          ...
          InkWell(
            onTap: () {
              ...
              addNewCustomer(context, customerList[index].displayName!,
                ...
                children: <Widget>[
                  Expanded(
                    ...
                    child: Container(
                      ...
                      width: 40,
                      child: Center(
                        child: Text(customerList[index].displayName![0],
                            style:
  ...
  getTransactionView(Customer customer) {
    ...
    if (ledger != null &&
      ...
      String message =
          ledger.lastPayType == Payment.GET ? 'Payment added ' : 'Gave credit';
      String formattedAmount = NumberFormat.currency(symbol: "\$").format(
          ledger.lastPayAmount != null ? ledger.lastPayAmount?.abs() : 0);
      return Text(
  ...
  void showSearchedContactsToTheList(String value) {
    setState(() async {
      ...
      tempCustomerList.retainWhere((element) =>
          element.displayName!.toUpperCase().contains(value.toUpperCase()));
      if (value.isNotEmpty) {
        var contacts = await ContactsService.getContacts(
            withThumbnails: false, query: value);
